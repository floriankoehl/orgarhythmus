...
export default function DependencyIOPopup({
  ...
  // ─── Inspect mode ─────────────────────────────────
  const [inspectData, setInspectData] = useState(null); // { scenarioId, scenario, sections }
  // ─── Expand mode ──────────────────────────────────
  const [expanded, setExpanded] = useState(false);
  // ─── Settings mode ────────────────────────────────
  const [settingsMode, setSettingsMode] = useState(false);
  const { settings: ownSettings, update: updateSettings } = usePromptSettings();
  // Use own (always-fresh) settings once loaded, fall back to prop while loading
  ...
  const [popupDims, setPopupDims] = useState({ width: 780, height: 700 });
  ...
  const buildChangeItemsWithCtx = useCallback((det) => {
    return buildDepChangeItems(det, ctx.nodes || {}, ctx.rows || {}, ctx.lanes || {}, ctx.edges || []);
  }, [ctx.nodes, ctx.rows, ctx.lanes, ctx.edges]);
  ...
  }, [assemblePrompt, modCtx, effectiveSettings, onReviewStart, buildChangeItemsWithCtx, customAddOn]);
    ...
    <>
      ...
      <div
        ...
        style={expanded ? {
          ...
          height: "94vh",
          display: hidePopup ? "none" : undefined,
        } : {
