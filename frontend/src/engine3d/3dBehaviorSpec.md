...
src/engine3d/
  ...
  floor3DMapping.js     — Pure mapping between world/board/entity coordinate spaces
  useFloor3D.js         — Floor representation hook: entity registry + hit-testing
...
## Floor Entity Registry (useFloor3D)

`useFloor3D` computes and exposes world-space bounds for every interactive entity on the floor:
...
### Hit-testing
`hitTest(worldX, worldZ)` returns `{ teamId, taskId, dayIndex }` for a world-space point.
- Returns `null` fields if the point is outside any entity area.
- Day column is computed from the Z coordinate within the day area of the board.
