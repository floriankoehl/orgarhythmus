...
/**
 * Modal that shows task/team JSON for copying or downloading.
 *
 * Supports multiple export scopes:
 *   - Full project: all teams + tasks
 *   - Selected teams: specific teams + their tasks
...
export default function TaskExportModal({ json, title, onClose }) {
  ...
  const handleDownload = useCallback(() => {
    ...
    const safeName = (title || "task_structure").replace(/[^a-zA-Z0-9_-]/g, "_").toLowerCase();
    ...
    <>
      ...
      >
        ...
        <div className="px-4 pb-3 text-[10px] text-gray-400">
          Tip: Copy this JSON, refine it with an AI model, then import it back to create teams &amp; tasks.
        </div>
      </div>
    </>
  );
