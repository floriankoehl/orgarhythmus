// ─────────────────────────────────────────────────────
// SafetyCheckPanel — Displays results of a scheduling
// safety check as a modal overlay.
// ─────────────────────────────────────────────────────
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          {results && !results.error && results.totalIssues === 0 && !isRunning && (
            <div className="flex flex-col items-center justify-center py-12 text-slate-400">
              ...
              <p className="text-xs text-slate-400 mt-1">All dependencies, deadlines, and project boundaries are respected.</p>
            </div>
          )}
                        ...
                        {cat.issues.map((issue, idx) => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {issue.sourceTask && issue.targetTask && (
                                <p className="text-[10px] text-slate-400 mt-0.5">
                                  {issue.sourceTask.name} → {issue.targetTask.name}
                                  {issue.dependency?.weight && (
                                    <span className="ml-1 text-slate-300">({issue.dependency.weight})</span>
                                  )}
                                </p>
                              )}
                              {issue.task && !issue.targetTask && (
                                <p className="text-[10px] text-slate-400 mt-0.5">
                                  Task: {issue.task.name}
                                </p>
                              )}
                            </div>
        ...
        <div className="px-6 py-3 border-t border-slate-100 flex items-center justify-between">
          <p className="text-[10px] text-slate-400">
            ...
            Click any issue to locate it on the timeline.
          </p>
          <button
            onClick={onClose}
